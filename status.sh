...
MY_STATUS_TITLE="My Website status page"

# Link for the homepage button
MY_HOMEPAGE_URL="http://cv.jmacias.com"
...
# Percentile to use for extract of timing
MY_TIMING_PERCENTILE="99"
...
MY_HOSTNAME_TIMING_HISTORY="$MY_STATUS_CONFIG_DIR/timing_hostname_history.txt"
MY_HOSTNAME_TIMING_HISTORY_TEMP_SORT="/tmp/timing_hostname_history_sort.txt"
...
debug_variables() {
	...
	echo "MY_HOSTNAME_TIMING_HISTORY: $MY_HOSTNAME_TIMING_HISTORY"
	echo
...
function echo_do_not_edit() {
	...
	echo "#     $MY_HOSTNAME_TIMING_HISTORY"
	echo "#"
}
...
function save_timing() {
	...
	MY_TIMING="$4"
	MY_DATE_TIME="$5"
	if cp "$MY_HOSTNAME_TIMING_HISTORY" "$MY_HOSTNAME_TIMING_HISTORY_TEMP_SORT" &> /dev/null; then
		printf "\n%s;%s;%s;%s;%s" "$MY_COMMAND" "$MY_HOSTNAME" "$MY_PORT" "$MY_TIMING" "$MY_DATE_TIME" > "$MY_HOSTNAME_TIMING_HISTORY"
		cat "$MY_HOSTNAME_TIMING_HISTORY_TEMP_SORT" >> "$MY_HOSTNAME_TIMING_HISTORY"
		rm "$MY_HOSTNAME_TIMING_HISTORY_TEMP_SORT" &> /dev/null
	else
		exit_with_failure "Can not copy file '$MY_HOSTNAME_TIMING_HISTORY' to '$MY_HOSTNAME_TIMING_HISTORY_TEMP_SORT'"
	fi
	
	if [[ "$BE_LOUD" = "yes" ]]; then
		printf "\n%-10s %-4s => %s sec" "TIMING:" "$MY_HOSTNAME" "$MY_TIMING"
		if [[ $MY_COMMAND == "nc" ]]; then
			printf " %s" "$(port_to_name "$MY_PORT")"
		fi
		if [[ $MY_COMMAND == "grep" ]]; then
...
function item_timing() {
	cat << EOF
<li class="list-group-item">
	<span class="badge">
EOF

	if [[ $MY_TIMING_VALUE > 0 ]]; then
		printf "%0.f ms" "$(echo "$MY_TIMING_VALUE * 1000" | bc)"
	fi

	echo "</span>"
	
	echo '<small class="text-muted">'
	echo "$MY_TIMING_DATE_TIME"
	echo '</small>'
...
check_file "$MY_HOSTNAME_TIMING_HISTORY"
check_file "$MY_HOSTNAME_TIMING_HISTORY_TEMP_SORT"
...
while IFS=';' read -r MY_COMMAND MY_HOSTNAME MY_PORT || [[ -n "$MY_COMMAND" ]]; do
	...
	elif [[ "$MY_COMMAND" = "percentile" ]]; then
		let MY_HOSTNAME_COUNT++
		MY_TIMING=`./ntimes 10 -- curl -s -o /dev/null -w '%{time_starttransfer}\n' --no-buffer -fs --max-time "$MY_TIMEOUT" "$MY_HOSTNAME" -H 'Accept-Encoding: gzip, deflate, sdch' -H 'Accept-Language: en-US,en;q=0.8,ja;q=0.6' -H 'Upgrade-Insecure-Requests: 1' -H 'User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.86 Safari/537.36' -H 'Connection: keep-alive' --compressed | ./percentile | grep "$MY_TIMING_PERCENTILE"'%'`
		NOTHING=''
		
		if [[ ! -z "$MY_TIMING" ]]; then
			MY_TIMING_NUMBER=`echo ${MY_TIMING/$MY_TIMING_PERCENTILE%\:/} | xargs`
			check_downtime "$MY_COMMAND" "$MY_HOSTNAME" "$MY_PORT"
			save_timing  "$MY_COMMAND" "$MY_HOSTNAME" "$MY_PORT" "$MY_TIMING_NUMBER" "$MY_DATE_TIME"
		else
			check_downtime "$MY_COMMAND" "$MY_HOSTNAME" "$MY_PORT"
		fi
	fi
...
# Get timing (last 20 extracts)
MY_TIMING_COUNT=0
MY_TIMING_ITEMS=()
while IFS=';' read -r MY_TIMING_COMMAND MY_TIMING_HOSTNAME MY_TIMING_PORT MY_TIMING_VALUE MY_TIMING_DATE_TIME || [[ -n "$MY_TIMING_COMMAND" ]]; do
	
	if [[ "$MY_TIMING_COMMAND" = "percentile" ]]; then
		let MY_TIMING_COUNT++
		MY_TIMING_ITEMS+=("$(item_timing)")
	fi
	if [[ "$MY_TIMING_COUNT" -gt "19" ]]; then
		break
	fi
	
done <"$MY_HOSTNAME_TIMING_HISTORY"

# History to HTML
if [[ "$MY_TIMING_COUNT" -gt "0" ]]; then
	cat >> "$MY_STATUS_HTML" << EOF
<div class="page-header">
	<h2>Past timings ($MY_TIMING_PERCENTILE% percentile)</h2>
</div>
<ul class="list-group">
EOF
	for MY_TIMING_ITEM in "${MY_TIMING_ITEMS[@]}"; do
		echo "$MY_TIMING_ITEM" >> "$MY_STATUS_HTML"
	done
	echo "</ul>" >> "$MY_STATUS_HTML"
fi
